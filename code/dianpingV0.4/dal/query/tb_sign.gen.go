...
func newTbSign(db *gorm.DB) tbSign {
	_tbSign := tbSign{}

	_tbSign.tbSignDo.UseDB(db)
	...
	_tbSign.Month = field.NewInt8(tableName, "month")
	_tbSign.Date = field.NewTime(tableName, "date")
	_tbSign.IsBackup = field.NewUint8(tableName, "is_backup")
...
func (t *tbSign) updateTableName(table string) *tbSign {
	...
	t.Month = field.NewInt8(table, "month")
	t.Date = field.NewTime(table, "date")
	t.IsBackup = field.NewUint8(table, "is_backup")
...
func (t tbSignDo) Exists(subquery interface{ UnderlyingDB() *gorm.DB }) ITbSignDo {
