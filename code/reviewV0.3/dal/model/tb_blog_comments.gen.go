...
type TbBlogComment struct {
	...
	BlogID     uint64    `gorm:"column:blog_id;type:bigint unsigned;not null;comment:探店id" json:"blog_id"`                             // 探店id
	ParentID   uint64    `gorm:"column:parent_id;type:bigint unsigned;not null;comment:关联的1级评论id，如果是一级评论，则值为0" json:"parent_id"`       // 关联的1级评论id，如果是一级评论，则值为0
	AnswerID   uint64    `gorm:"column:answer_id;type:bigint unsigned;not null;comment:回复的评论id" json:"answer_id"`                      // 回复的评论id
	Content    string    `gorm:"column:content;type:varchar(255);not null;comment:回复的内容" json:"content"`                               // 回复的内容
	Liked      uint32    `gorm:"column:liked;type:int unsigned;comment:点赞数" json:"liked"`                                              // 点赞数
	Status     uint32    `gorm:"column:status;type:tinyint unsigned;comment:状态，0：正常，1：被举报，2：禁止查看" json:"status"`                       // 状态，0：正常，1：被举报，2：禁止查看
